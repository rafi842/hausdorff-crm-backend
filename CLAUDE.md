...
**Database is better-sqlite3** (migrated from sql.js — the file format is identical, so the same `crm.db` works under either). [`backend/database.js`](backend/database.js) opens the file directly and writes each statement incrementally — no whole-file rewrite. The `run/get/all` wrappers are the only API; they normalise params (`undefined`→null, `boolean`→0/1) because better-sqlite3 rejects what sql.js tolerated. Implications:
- Writes are durable-on-write; `saveDb()` is a no-op kept only so callers don't break. `getDb()` is exported but unused.
- Writes are serialised and safe — better-sqlite3 is synchronous. Transactions are available (`db.transaction()`) though not currently used.
- Journal mode is still the default (rollback), **not** WAL — a deliberately deferred step (WAL needs the backup cron to checkpoint first).
- Schema is created and migrated in `initializeDatabase()` / `runMigrations()` in the same file. Migrations are idempotent `ALTER TABLE` calls wrapped in try/catch that swallow "column already exists" errors. To add a column: append to the `migrations` array in [`backend/database.js`](backend/database.js) — do **not** edit the original `CREATE TABLE`. DDL inside `initializeDatabase()` uses `db.exec(...)`, not the `run` wrapper.
- DB lives at `backend/crm.db` by default (override with `DB_PATH`). On first boot the engine writes a one-time `crm.db.pre-better-sqlite3.bak` beside it. A nightly cron in `server.js` copies it into `backend/backups/`.
- **Native module:** better-sqlite3 compiles/prebuilds per platform. `node_modules` is gitignored, so Railway rebuilds it on deploy — a failed native build is the one thing that blocks a deploy.
